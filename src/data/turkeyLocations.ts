...
export const TURKEY_PROVINCES: ProvinceOption[] = [
...
const districtCache = new Map<number, string[]>();

export async function getDistricts(provinceName: string): Promise<string[]> {
  const province = TURKEY_PROVINCES.find((item) => item.name === provinceName);
  if (!province) return [];
  const cached = districtCache.get(province.id);
  if (cached) return cached;
  const response = await fetch(
    `https://api.turkiyeapi.dev/v2/districts?provinceId=${province.id}&fields=id%2Cname&limit=100`,
  );
  if (!response.ok) throw new Error("İlçe listesi yüklenemedi.");
  const body = await response.json() as { data?: Array<{ name?: string }> };
  const districts = (body.data ?? []).map((item) => item.name?.trim() ?? "").filter(Boolean);
  districtCache.set(province.id, districts);
  return districts;
}
