...
export function AuditLogsPage() {
  ...
  return <>
    ...
    <div className="toolbar"><label className="table-search"><Search size={16} /><input placeholder="Kayıtlarda ara" /></label><button className="button button--outline" onClick={load}><RefreshCw size={15} /> Yenile</button></div>
    <section className="data-card"><div className="data-table audit-table"><div className="data-row data-head"><span>İŞLEM</span><span>ROL</span><span>KAYNAK</span><span>İSTEK KİMLİĞİ</span><span>IP ADRESİ</span><span>TARİH</span></div>
      ...
      {!loading && logs.length === 0 && <div className="empty-state"><ShieldCheck /><b>Kayıt bulunamadı</b><span>İlk işlemlerden sonra burada görünecek.</span></div>}
    </div></section>
  </>;
}
