...
export function BalancesPage() {
  ...
  const visibleTransactions = useMemo(() => {
    ...
    return needle ? transactions.filter((item) => `${item.id} ${item.tenantName} ${item.description} ${item.reference} ${item.direction}`.toLocaleLowerCase("tr-TR").includes(needle)) : transactions;
  }, [query, transactions]);
  ...
  function openAdjustment() {
    ...
    setCompanyType(type);
    setForm({ tenantId: wallets.find(wallet => wallet.tenantType === type)?.tenantId ?? "", amount: "", description: "", reference: "" });
    setError("");
    setModalOpen(true);
  }
  return <>
    ...
    {isPlatform && modalOpen && <div className="modal-wrap"><button className="modal-backdrop" onClick={() => setModalOpen(false)} aria-label="Kapat" /><form className="tenant-modal compact-modal" onSubmit={adjust}><div className="modal-head"><div><p className="eyebrow">FİNANSAL İŞLEM</p><h2>Bakiye ekle veya düş</h2><span>Eksi tutar bakiyeden düşer; tüm işlemler loglanır.</span></div><button type="button" className="icon-button" onClick={() => setModalOpen(false)}><X /></button></div><div className="modal-fields"><label><span>Firma tipi</span><select value={companyType} onChange={event => { const type = event.target.value as "CONTRACTOR" | "CPO"; setCompanyType(type); setForm({ ...form, tenantId: wallets.find(wallet => wallet.tenantType === type)?.tenantId ?? "" }); }}><option value="CONTRACTOR">Taşeron</option><option value="CPO">CPO</option></select></label><label><span>{companyType === "CONTRACTOR" ? "Taşeron firma" : "CPO firma"}</span><select required value={form.tenantId} onChange={event => setForm({ ...form, tenantId: event.target.value })}><option value="">Firma seçin</option>{selectableWallets.map(wallet => <option value={wallet.tenantId} key={wallet.id}>{wallet.tenantName}</option>)}</select></label><label><span>Tutar (+ / -)</span><input required type="number" value={form.amount} onChange={event => setForm({ ...form, amount: event.target.value })} /></label><label><span>Açıklama</span><select required value={form.description} onChange={event => setForm({ ...form, description: event.target.value })}><option value="">İşlem türü seçin</option><option>Hak ediş ödemesi</option><option>Bakiye yükleme</option><option>Düzeltme</option><option>İade</option></select></label><label><span>Referans</span><input value={form.reference} onChange={event => setForm({ ...form, reference: event.target.value })} /></label></div>{error && <div className="login-error">{error}</div>}<div className="modal-actions"><button type="button" className="button button--outline" onClick={() => setModalOpen(false)}>Vazgeç</button><button className="button button--primary">İşlemi kaydet</button></div></form></div>}
  </>;
}
