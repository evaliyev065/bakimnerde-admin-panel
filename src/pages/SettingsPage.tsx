...
const emptyForm = { name: "", tenantKey: "", type: "CPO" as TenantType, contactEmail: "", contactPhone: "", adminName: "", adminEmail: "", adminPassword: "" };
...
export function SettingsPage() {
  ...
  return <>
    ...
    <section className="security-banner"><span><ShieldCheck /></span><div><h2>Tenant izolasyonu etkin</h2><p>CPO ve taşeron firmalar yalnız kendi işlerini görür; özel maliyet ve politikaları Bakımnerde yönetir.</p></div><strong><Check size={15} /> Korumalı</strong></section>
    <div className="settings-grid">
      ...
      <aside className="settings-side">
        ...
        <article className="panel permission-card"><span><UserCog /></span><div><h3>3+1 erişim modeli</h3><p>Bakımnerde, CPO, taşeron yönetimi ve yalnız mobil saha rolleri ayrıdır.</p></div><b>Etkin</b></article>
    ...
    <section className="role-matrix panel"><div className="panel__head"><div><h2>Rol erişim özeti</h2><p>Tarafların panelde görebildiği alanlar</p></div><Building2 size={19} /></div><div className="matrix"><div><b>YETKİ</b><b>BAKIMNERDE</b><b>CPO</b><b>TAŞERON YÖNETİMİ</b></div>{[["Tüm işleri görüntüleme","✓","—","—"],["Firma özel fiyatlarını yönetme","✓","—","—"],["Kendi işlerini görüntüleme","✓","✓","✓"],["Bakım kanıtlarını görüntüleme","✓","✓","✓"],["Bakiye işlemi yapma","✓","—","—"]].map(row => <div key={row[0]}>{row.map((cell, index) => <span className={cell === "✓" ? "allowed" : ""} key={index}>{cell}</span>)}</div>)}</div></section>
